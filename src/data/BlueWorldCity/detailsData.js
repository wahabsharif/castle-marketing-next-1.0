export const detailsData = [
  {
    id: 1,
    image: "proximity-blue-world-city.webp",
    title: "Location",
    text: "Strategically positioned on the main Chakri Road, Blue World City Islamabad provides direct access from the M-2 Lahore-Islamabad Motorway through the Chakri Interchange. Its prime location ensures close proximity to both Islamabad and Rawalpindi, along with easy access to the new International Islamabad Airport. The housing society has earned acclaim from investors and realtors for its scenic surroundings and tranquil atmosphere, providing residents with a luxurious and serene lifestyle away from the urban hustle and bustle.",
  },
  {
    id: 2,
    image: "master-plan-blue-world-city.webp",
    title: "Master Plan",
    text: "The master plan of Blue World City Islamabad is a meticulously crafted collaboration with national and international experts, incorporating the latest principles of urban town planning. This ensures a well-designed and modern development that aligns with contemporary standards in city planning.",
  },
  {
    id: 3,
    image: "blue-world-city-features.webp",
    title: "Feature & Amenities",
    text: "Blue World City Islamabad, a Pak-China Friendly City, offers an array of features and amenities. The master plan, crafted with national and international expertise, ensures modern urban development. Tourist attractions, a Blue World Economic Zone, a hilltop 5-star hotel, and a 208-feet wide Central Avenue contribute to a luxurious lifestyle.",
  },
];
